...
export class LtiHandler {
    ...
    handleLogin(req, res) {
        ...
        console.log("login request from", params.iss, "with client id", params.client_id); // may be remove later
        let platform = this.lmslist[params.iss];
        if (!platform) return res.status(400).send({ status: 400, error: 'Platform not registered!' });
    ...
    async handleLaunch(req, res) {
        ...
        if (req.body.id_token) {
            const decoded_token = jwt.decode(req.body.id_token, { complete: true });
            const platform = this.lmslist[decoded_token.payload.iss];
            if (!platform) return res.status(400).send({
                status: 400, error: 'Bad Request',
                details: { message: 'platform not registered/supported' }
            });

            let keyinfo = await got.get(platform.keyset_url).json();
            let keys = keyinfo.keys;
            if (!keys) return res.status(400).send({ status: 400, error: 'Keyset not found' });

            if (!decoded_token.header.kid)
                return res.status(400).send({ status: 400, error: 'no valid kid!' });

            const jwk = keys.find(key => { return key.kid === decoded_token.header.kid });
            if (!jwk) return res.status(400).send({ status: 400, error: 'key not found' });
            const key = await Jwk.export({ jwk: jwk });
            let payload = decoded_token.payload;

            //console.log("decoded token payload",payload);
            if (!validate(payload.nonce)) return res.status(400).send({ status: 400, error: 'nonce wrong format' });
            this.redis.exists("lti:nonce:" + payload.nonce, async (err, redres) => {
                if (err) {
                    console.log("redis error", err);
                    return res.status(400).send({ status: 400, error: 'redis broken' });
                } else if (redres == 1) return res.status(400).send({ status: 400, error: 'nonce reused, replay attack?' });
                else {


                    // we have to store it in the db
                    this.redis.set("lti:nonce:" + payload.nonce, "dummy", 'EX', 60 * 10 /* 10 minutes*/); // we do not have to use the callback

                    if (!jwt.verify(req.body.id_token, key, {/* nonce: ADD*/ }))
                        return res.status(400).send({ status: 400, error: 'jwt verification failure' });

                    if (payload['https://purl.imsglobal.org/spec/lti/claim/message_type'] !== 'LtiResourceLinkRequest')
                        return res.status(400).send({status: 400, 
                            error: "so far only resource links are supported " 
                    ...
                    if (payload['https://purl.imsglobal.org/spec/lti/claim/ext'])
                        userinfo.lmsusername =  payload['https://purl.imsglobal.org/spec/lti/claim/ext'].user_username;

                    //console.log("userinfo", userinfo);
                    let lmscontext = {
                        // TODO add unique platform identifier?
                        ret_url: payload['https://purl.imsglobal.org/spec/lti/claim/launch_presentation'].return_url,
                        iss: payload.iss, //not optional, use for identification 

                        /*aud: payload.aud,*/ //may be exclude
                        platform_id: payload['https://purl.imsglobal.org/spec/lti/claim/tool_platform'].guid, //optional
                        deploy_id: payload['https://purl.imsglobal.org/spec/lti/claim/deployment_id'], // do not use for identification, period!
                        course_id: payload['https://purl.imsglobal.org/spec/lti/claim/context'].id, //optional, use for context identification if possible
                        resource_id: payload['https://purl.imsglobal.org/spec/lti/claim/resource_link'].id, // use for identification

                    }
                    if ( !lmscontext.deploy_id
                        || !lmscontext.resource_id || !lmscontext.iss) 
                        return res.status(400).send({ status: 400, error: 'lti mandatory fields missing' });
                    ...
                    let lectureinfo = {
                        coursetitle: payload['https://purl.imsglobal.org/spec/lti/claim/context'].title ||
                            ...
                            payload['https://purl.imsglobal.org/spec/lti/claim/resource_link'].title,
                        lecturetitle: payload['https://purl.imsglobal.org/spec/lti/claim/resource_link'].title
                    }
                    ...
                    if (payload['https://purl.imsglobal.org/spec/lti/claim/roles'].includes('http://purl.imsglobal.org/vocab/lis/v2/membership#Learner'))
                        role.push('audience');

                    if (payload['https://purl.imsglobal.org/spec/lti/claim/roles'].includes('http://purl.imsglobal.org/vocab/lis/v2/membership#Instructor')){
                        if (payload.sub) {
                    ...
                    // and the user !!
                    let failsuser= await this.identifyCreateUser(userinfo);
                    console.log("failsuser",failsuser);
                    let courseinfo={lms: lmscontext,
                        linfo: lectureinfo};
                    ...
                    let failscourse=await this.identifyCreateLectureAndCourse(courseinfo); // TODO
                    if (!failscourse) return res.status(400).send({ status: 400, error: 'resource can not be identified' });
                    console.log("failscourse",failscourse);

                    let token= {course: failscourse, user: failsuser, role: role, context:"lti", maxrenew: 5}; //five times 5 minutes should be enough
                    let jwttoken=await this.signJwt(token);
                ...
                res.send('LTI something failed');


                //console.log("decoded token",decoded_token.payload);
            });
    ...
    {
        let userscol=this.mongo.collection("users");

        let orquery=[];
        if (userinfo.lmssub) orquery.push({"lms.sub": userinfo.lmssub });
        if (userinfo.lmsusername) orquery.push({"lms.username": userinfo.lmsusername});
        if (userinfo.email) orquery.push({email:userinfo.email});

        if (orquery.length==0) return; // no user info get out

        let userdoc=await userscol.findOne({ $or: orquery});
        ...
        if (userdoc == null) { // deploy data
            let toinsert={};
            if (!displayname && firstname && lastname) displayname=firstname+lastname;
            ...
            let toupdate={};
            if (userdoc.displayname!=displayname) toupdate.displayname=displayname;
            if (userdoc.firstnames!=userinfo.firstnames) toupdate.firstnames=userinfo.firstnames;
            if (userdoc.lastname!=userinfo.lastname) toupdate.lastname=userinfo.lastname;
            if (userdoc.email!=userinfo.email) toupdate.email=userinfo.email;
            if ((userinfo.lmssub || userinfo.lmsusername) 
                    && (!userdoc.lms || userdoc.lms.sub!=userinfo.lmssub ||
            ...
            if (Object.keys(toupdate).length>0){
                    userscol.updateOne({uuid: useruuid},
                        {$set: toupdate, $currentDate: {lastlogin: true}});
            } else {
               // console.log("no update",toupdate);
               if (!userdoc.lastlogin || (userdoc.lastlogin && moment(userdoc.lastlogin).isBefore(moment().subtract(3,'days')) )) {
    ...
    async identifyCreateLectureAndCourse(args)
    {
        ...
        let lecturedoc=await lecturescol.findOne({$and: andquery});
        ...
        if (lecturedoc == null) { // deploy data
            let toinsert={};
            toinsert.lms={};
            ...
            if (args.owner) {
                ...
                if (args.ownerdisplayname) toinsert.ownersdisplaynames=[args.ownerdisplayname];
            ...
            //check if we want to update
            let toupdate={};
            if (lecturedoc.lms.course_id!=lms.course_id|| lecturedoc.lms.platform_id!=lms.platform_id) toupdate.lms={};
            if (lecturedoc.lms.course_id!=lms.course_id)  toupdate.lms.course_id=lms.course_id;
            if (lecturedoc.lms.platform_id!=lms.platform_id)  toupdate.lms.platform_id=lms.platform_id;
            if (lecturedoc.title!=linfo.lecturetitle) toupdate.title=linfo.lecturetitle;
            if (lecturedoc.coursetitle!=linfo.coursetitle) toupdate.coursetitle=linfo.coursetitle;
            ...
            if (Object.keys(toupdate).length>0 || !containsowner) {
               ...
               if (!containsowner && isowner) {
                   ...
                   if (args.ownerdisplayname) updateops.$push={ownersdisplaynames: args.ownerdisplayname};
               }
               ...
               lecturescol.updateOne({uuid: lectureuuid},updateops);
                ...
                if (!lecturedoc.lastaccess || (lecturedoc.lastaccess && moment(lecturedoc.lastaccess).isBefore(moment().subtract(3,'days')) )) {
                    if (isowner) {
        ...
        let retobj={lectureuuid: lectureuuid};
